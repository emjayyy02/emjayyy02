<div align="center">

# 👋 Hi, I'm Marvin!


*Building things, breaking things, learning how to fix them, and enjoying the process.*

</div>

---

## 🌱 A little about me

I'm **Marvin Silverio**, a 19-year-old 2nd year BSIS college student from the Philippines 🇵🇭.

Right now, I'm exploring the world of **AI automation, web development, and software development**. My long-term goal is to become a **Full-Stack Web Developer & Automation Engineer**.

I'm definitely not there yet, but that's kinda the point.

This GitHub will be the **eyewitness of the journey**. From small exercises and experiments to projects I'm genuinely proud of.

---

## ⚡ What I'm up to

* 🤖 Learning and building **AI automations & workflows**
* 🌐 Working my way toward **full-stack web development**
* 🐍 Building projects with **Python**
* ☕ Exploring and improving my **Java / programming fundamentals**
* 🛠️ Creating projects that solve actual problems, not just tutorial projects
* 📚 Learning one project, mistake, and random bug at a time

---

## 🏔️ The journey

<div align="center">

![Mountain Gazer](mountain.png)

*No rush. Just keep climbing.*

</div>

Some days I build something cool.

Some days I spend two hours fixing something that was caused by one missing character.

Either way, **progress is progress.** 😭

---

## 🔗 Let's connect

Want to talk about tech, automation, projects, opportunities, or just say hi?

**Links coming soon...** 👀

---

<div align="center">

![Miles Morales](miles.png)

### ⭐ Thanks for stopping by!

*"Hope is a good thing, maybe the best of things, and no good thing ever dies."*
**— Andy Dufresne, The Shawshank Redemption**

</div>


